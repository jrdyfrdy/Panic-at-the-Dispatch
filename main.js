...
// Function to synthesize a terrifying heartbeat that gets faster
function playHeartbeat(timeProgress) {
  if (!devConfig.audioEnabled || audioCtx.state === "suspended") return;
  const now = audioCtx.currentTime;
  if (now < nextHeartbeat) return;

  // Faster as time runs out (from 1.5s interval down to 0.2s interval)
  const interval = Math.max(1.5 - timeProgress * 1.5, 0.2);
  nextHeartbeat = now + interval;

  const osc = audioCtx.createOscillator();
  const gain = audioCtx.createGain();
  osc.type = "sine";
  osc.frequency.setValueAtTime(60, now); // Deep thump
  osc.frequency.exponentialRampToValueAtTime(10, now + 0.1);

  gain.gain.setValueAtTime(0, now);
  gain.gain.linearRampToValueAtTime(0.8 * timeProgress, now + 0.05); // Gets louder as panic sets in
...
const savedSettings =
  JSON.parse(localStorage.getItem("tatag_dev_settings")) || {};
const devConfig = { ...defaultDevConfig, ...savedSettings };
...
scene.fog = new THREE.Fog(
  COLOR_WHITE,
  10,
  devConfig.mazeSize * devConfig.cellSize * 0.8,
);
...
const rescueSound = {
  play: async () => {
    ...
    osc.frequency.setValueAtTime(1046.5, time + 0.45); // C6
...
function initMaze() {
  ...
  const groundGeo = new THREE.PlaneGeometry(
    devConfig.mazeSize * devConfig.cellSize * 2,
    devConfig.mazeSize * devConfig.cellSize * 2,
  );
  ...
  while (!solvable) {
    ...
    for (let dz = -1; dz <= 1; dz++) {
      for (let dx = -1; dx <= 1; dx++) {
        ...
        if (
          sz > 0 &&
          sz < devConfig.mazeSize - 1 &&
          sx > 0 &&
          sx < devConfig.mazeSize - 1
        ) {
          grid[sz][sx] = 0;
        }
        ...
        if (
          ez > 0 &&
          ez < devConfig.mazeSize - 1 &&
          ex > 0 &&
          ex < devConfig.mazeSize - 1
        ) {
          grid[ez][ex] = 0;
        }
      }
  ...
  const targetWire = new THREE.LineSegments(
    new THREE.EdgesGeometry(targetGeoOuter),
    wireframeMaterial,
  );
...
function generateGrid() {
  ...
  for (let z = 0; z < devConfig.mazeSize; z++) {
    ...
    for (let x = 0; x < devConfig.mazeSize; x++) {
      ...
      if (
        x === 0 ||
        x === devConfig.mazeSize - 1 ||
        z === 0 ||
        z === devConfig.mazeSize - 1
      ) {
        row.push(1);
      } else {
  ...
  let cx = 1;
  let cz = 1;
  let ex = devConfig.mazeSize - 2;
  let ez = devConfig.mazeSize - 2;
  while (cx !== ex || cz !== ez) {
    grid[cz][cx] = 0;
    if (Math.random() > 0.5) {
      if (cx < ex) cx++;
...
function checkSolvable(start, end) {
  const queue = [
    { x: start.x, z: start.z, path: [{ x: start.x, z: start.z }] },
  ];
  const visited = Array(devConfig.mazeSize)
    .fill(0)
    .map(() => Array(devConfig.mazeSize).fill(false));
  ...
  while (queue.length > 0) {
    ...
    for (let [dz, dx] of dirs) {
      ...
      if (
        nz >= 0 &&
        nz < devConfig.mazeSize &&
        nx >= 0 &&
        nx < devConfig.mazeSize
      ) {
  ...
  for (let z = 0; z < devConfig.mazeSize; z++) {
    for (let x = 0; x < devConfig.mazeSize; x++) {
      if (grid[z][x] === 1) {
        ...
        const bHeight =
          Math.random() > 0.7
            ? WALL_HEIGHT * (1.2 + Math.random() * 1.5)
            : WALL_HEIGHT;
        ...
        const randMat =
          procBuildingMaterials[
            Math.floor(Math.random() * procBuildingMaterials.length)
          ].clone();
...
document.addEventListener("keydown", (event) => {
  switch (event.code) {
    ...
    case "KeyD":
      ...
      break;
    case "KeyE":
      if (event.shiftKey) exportCSV();
      break;
    case "KeyR":
      if (event.shiftKey) resetData();
      break;
    case "KeyP":
      if (event.shiftKey) openDevSettings();
      break;
    case "KeyM":
      // Only toggle if the Control key is being held down
      if (event.shiftKey) {
        // Optional: Prevent default browser behavior for Ctrl+M just in case
        event.preventDefault();

        devConfig.minimapEnabled = !devConfig.minimapEnabled;
        minimapCanvas.style.display = devConfig.minimapEnabled
          ? "block"
          : "none";
...
function checkCollision(targetPosition, pr = 0.8) {
  ...
  const minXGrid = Math.floor(
    (targetPosition.x - pr) / devConfig.cellSize + devConfig.mazeSize / 2 + 0.5,
  );
  const maxXGrid = Math.floor(
    (targetPosition.x + pr) / devConfig.cellSize + devConfig.mazeSize / 2 + 0.5,
  );
  const minZGrid = Math.floor(
    (targetPosition.z - pr) / devConfig.cellSize + devConfig.mazeSize / 2 + 0.5,
  );
  const maxZGrid = Math.floor(
    (targetPosition.z + pr) / devConfig.cellSize + devConfig.mazeSize / 2 + 0.5,
  );
  ...
  for (let z = minZGrid; z <= maxZGrid; z++) {
    for (let x = minXGrid; x <= maxXGrid; x++) {
      // Boundary safety
      if (z < 0 || z >= devConfig.mazeSize || x < 0 || x >= devConfig.mazeSize)
        return true;
...
function drawMinimap() {
  ...
  const pGridX =
    camera.position.x / devConfig.cellSize + devConfig.mazeSize / 2;
  const pGridZ =
    camera.position.z / devConfig.cellSize + devConfig.mazeSize / 2;
  ...
  // --- ADD THIS AT THE END OF drawMinimap() ---

  // Draw the Monster (if it has spawned)
  if (monsterMesh) {
    const mGridX =
      monsterMesh.position.x / devConfig.cellSize + devConfig.mazeSize / 2;
    const mGridZ =
      monsterMesh.position.z / devConfig.cellSize + devConfig.mazeSize / 2;

    minimapCtx.fillStyle = "#000000"; // Pitch black core
    minimapCtx.beginPath();
    minimapCtx.arc(
      (mGridX + 0.5) * cellW,
      (mGridZ + 0.5) * cellH,
      cellW / 1.8, // Slightly larger than the player dot
      0,
      Math.PI * 2,
    );
    minimapCtx.fill();

    // Add a glowing red outline to match its 3D wireframe
...
function triggerGameOver(reason) {
  ...
  const mainTitle = reason === "TIME" ? "TIME IS UP!" : "IT CAUGHT YOU.";
  ...
  flashScreen.style.backgroundColor = "black";
  flashScreen.style.display = "flex";
  flashScreen.style.flexDirection = "column";
  flashScreen.style.justifyContent = "center";
  flashScreen.style.alignItems = "center";
  flashScreen.style.fontFamily = "monospace";
  flashScreen.style.zIndex = "9999";
  flashScreen.style.cursor = "pointer";
  flashScreen.style.userSelect = "none";
  ...
  const flashInterval = setInterval(() => {
    flashScreen.style.color = isRed ? "darkred" : "red";
    flashScreen.style.transform = isRed ? "scale(1.02)" : "scale(1)";
    isRed = !isRed;
  }, 100);
